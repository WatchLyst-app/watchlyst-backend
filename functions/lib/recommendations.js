...
exports.updateMovieFeatures = exports.getUserScoringDetails = exports.refreshRecommendations = exports.generateInitialRecommendations = exports.processSwipeInteraction = void 0;
...
exports.processSwipeInteraction = functions.firestore
    ...
    try {
        ...
        const scoreBefore = scoringSystem.calculateRecommendationScore(currentPreferences, movieFeatures);
        // 4. Update preferences
        const updateResult = scoringSystem.updateUserPreferences(currentPreferences, movieFeatures, action, learningMetadata.learningRate, learningMetadata.totalUpdates);
        // 5. Calculate scores after update
        const scoreAfter = scoringSystem.calculateRecommendationScore(updateResult.preferences, movieFeatures);
        // 6. Update user preferences in Firestore
        learningMetadata.totalUpdates += 1;
        learningMetadata.modelConfidence = scoringSystem.calculateModelConfidence(learningMetadata.totalUpdates);
...
exports.generateInitialRecommendations = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        moviesSnapshot.forEach(doc => {
            allMovies.push(Object.assign({ id: doc.id }, doc.data()));
        });
        ...
        const scoredMovies = allMovies.map(movie => {
            ...
            const scoreResult = scoringSystem.calculateRecommendationScore(initialPreferences, features);
            // Determine reason for recommendation
            let reason = 'exploration';
            let bonusScore = 0;
            // Category match bonus
            if (movie.genres && Array.isArray(movie.genres)) {
                const hasSelectedGenre = movie.genres.some((genre) => selectedCategories.some((cat) => genre.toLowerCase().includes(cat.toLowerCase())));
                if (hasSelectedGenre) {
        ...
        const targetDistribution = {
            category_match: Math.floor(50 * 0.6),
            trending: Math.floor(50 * 0.25),
        ...
        for (const reason of ['category_match', 'trending', 'exploration']) {
            const targetCount = targetDistribution[reason];
            let added = 0;
...
async function refreshUserRecommendations(userId) {
    ...
    try {
        ...
        const explorationRate = scoringSystem.calculateExplorationRate(userStats.totalSwipes || 0, userStats.likeRatio || 0.5, 0.15);
        ...
        moviesSnapshot.forEach(doc => {
            if (!seenMovies.has(doc.id)) {
                unseenMovies.push(Object.assign({ id: doc.id }, doc.data()));
            }
        });
        ...
        const scoredMovies = unseenMovies.map(movie => {
            ...
            // Calculate score
            const scoreResult = scoringSystem.calculateRecommendationScore(userPreferences, features, explorationRate, isExploration);
...
exports.refreshRecommendations = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Failed to refresh recommendations');
    }
});
// ===== HELPER FUNCTION: Get User Scoring Details =====
exports.getUserScoringDetails = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userId = context.auth.uid;
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Failed to get scoring details');
    }
});
// ===== SCHEDULED FUNCTION: Update Movie Features =====
exports.updateMovieFeatures = functions.pubsub
